...
//                df.format(SpeedOmeter.getDegPerSecond()));
        ...
        telemetry.addLine("\nLift raw encoder: " + myLift.getExtensionPosRAW());
        telemetry.addLine("Lift offset: " + myLift.masterOffsetTicks);

        telemetry.addLine("Hang mech curr percent: " +
                df.format(hangMechanism.getCurrentPercent()));
        telemetry.addLine("Hang mech curr position: " +
                df.format(hangMechanism.getCurrentPosition()));
...
//        tuneScalingFactorsWithTuner();
        ...
        telemetry.addLine("liftErrorSum: " + myLift.getCurrLiftErrorSum());
        tp5.markStart();
        super.loop();
        tp5.markEnd();
...
//            myAutoFeeder.addCollectingLocation(0.5 + (gamepad2.left_stick_x/5.0));
    ...
    private void ResetMyPositionWhenGamepad1X() {
        if(gamepad1.x){
            MyPosition.setPosition(140,140,Math.toRadians(-135));
...
//        telemetry.addLine("</font face='monospace'>");
    ...
    public void stop(){
        ...
        //rerunning teleop
        initialCollectorDumperPosition = 1.0;
    }
}
