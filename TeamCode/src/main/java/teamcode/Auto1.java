...
public class Auto1 extends Auto {
    ...
    private static final double compensateXBackHome = -3 - compensateXDrivingToMarker;
    private static final double compensateYBackHome = -3 - compensateYDrivingToMarker;//2.605
...
//        DrawVirtualField();
    ...
    public void start(){
        ...
        double leftCubePercent = 0.45 - 0.21;
        double rightCubePercent = 0.45 + 0.20;
        ...
        if(cubeLocation == 1){
            for(int i = 0; i < 60-2; i += 3){
                ...
                myAutoCollector.allCollectingLocations.get(i+1).pointLocationY = -25;

                myAutoCollector.allCollectingLocations.get(i+2).pointLocationY = 10;
        ...
        if(cubeLocation == 0){
            for(int i = 0; i < 60-2; i += 4){
                ...
                myAutoCollector.allCollectingLocations.get(i+3).pointLocationX = 65;
...
//        MyPosition.setPosition(147.1 + 1.33 - 1.1, 143.2-3+0.373,Math.toRadians(-45));
...
//        DrawVirtualField();
    ...
    private void ExtendIntoCraterWhenReady() {
        ...
        long parkingTime = 600;//time to allow the collector to extend into the crater
...
//                myLift.setCurrentPositionTicks(0);//calibrate the lift
    ...
    public void MainStateMachine() {
        ...
        if(programStage == progStates.unlatching.ordinal()){
            if(stageFinished){
                MyPosition.setPosition(147.1 + 1.33 - 1.1, 143.2-3+0.373,
                        Math.toRadians(-45));//( - 0.0525)
                initializeStateVariables();
            }
            //turn
        ...
        if(programStage == progStates.drivingToMarker.ordinal()){
            ...
            allPoints.add(new CurvePoint(190, 30.5,
                    0.5*SPEED_SCALE,0.5*SPEED_SCALE,50,65,
                    Math.toRadians(60),0.6));
...
//                myCollector.setCurrentPositionTicks(210);//GOING HOME
        ...
        if(programStage == progStates.getOurBall.ordinal()){
            ...
            double approachX = (-Math.cos(getAngle_rad()) * approachDist)+ballX;
            double approachY = (-Math.sin(getAngle_rad()) * approachDist)+ballY;
