    ...
    public void update(){
        ...
        if(myState == myStates.retractingCollectorAndMoveInLift.ordinal()){
            ...
            double collectorWithPrediction = myCollector.getExtensionPercent() +
                    myCollector.getExtensionCurrentSpeedPercent() * 0.025;//0.05
...
//                myLift.setCurrentPositionTicks(0);
            ...
            boolean activateRoller = collectorWithPrediction < 0.06
                    && myCollector.getTiltPercent() < 0.04 &&
                    liftIsDown &&
                    (isOkBecauseOfMoving || myCollector.getExtensionPercent() < 0.06) &&
                     elapsedTime > 350;
            if(activateRoller){
...
//                myCollector.reverseCollector();
                myCollector.manualControl();
                myCollector.setRollerPower(-0.8);
...
//            myCollector.reverseCollector();
        ...
        if(myState == myStates.movingUpLift.ordinal()){
            ...
            if(myLift.getExtensionPercent() > 0.67 &&
                    Math.abs(myRobot.getDeltaRadiusForFeeding()) < 5 &&
...
//            myLift.dump();
            ...
            int timeToWait = !earlyFinishDump ? 1350 : 700;//TODO: re-enable early dump
