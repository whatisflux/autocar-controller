...
public class Robot extends TunableOpMode {
    ...
    private LoadingSensors myLoadingSensors;
...
//    public PitScannerInterface myPitScannerInterface;
    ...
    public static double initialCollectorDumperPosition = 1.0;
...
//        myPitScannerInterface = new PitScannerInterface(this);
        ...
        Log.d("ERROR_LOG","dumper activated? " + initialCollectorDumperPosition);
        ...
        Log.d("ERROR_LOG","collector position: " + myCollector.dumperServoMasterPosition);
        Log.d("ERROR_LOG","has set servo: " + myCollector.hasEverSetServo);
        ...
        myLift = new Lift(this,liftMotor,encoderMotor,
                ...
                (ExpansionHubServo) hardwareMap.get("releaseServo"));
        ...
        myCollector.update();
        Log.d("ERROR_LOG","2collector position: " + myCollector.dumperServoMasterPosition);
        Log.d("ERROR_LOG","2has set servo: " + myCollector.hasEverSetServo);
//        Log.d("ERROR_LOG","state after: " + myCollector.dumperState);
...
//        giveCppFieldData();
        ...
        telemetry.addLine("has set servo: " + myCollector.hasEverSetServo);
        telemetry.addLine("dump curr pos: " + myCollector.dumperServoMasterPosition);
...
//        telemetry.addLine("profile 8: " + tp8.getAverageTimePerUpdateMillis());
    ...
    private void updateLoadingSensorsIfNecessary() {
        ...
        if(myCollector.rollerState == Collector.rollerStates.forwards &&
            ...
            //we will update them pretty fast if we are going forwards with the collector
            //and it is deployed
            if(currTimeMillis - myLoadingSensors.getLastUpdateTimeMillis() > 30){
                //since the collector is going forwards and we are deployed, update
...
//        myLift.DUMP_SERVO_DEACTIVATED = getDouble("tuneServo2");
...
//        myLift.RELEASE_SERVO_UNRELEASED = getDouble("unreleasedPosition");
    ...
    public void ControlMovement() {
        double masterScale = 0.5 + (gamepad1.left_trigger * (1.0-0.5));
    ...
    public static double craterAutoFeedDistance = 79.114 + 18;
    public static double craterTargetX = 175 + 15;
    public static double craterTargetY = 156 + 10;
    ...
    public static double depotTargetY = 165;
    ...
    public boolean driveToAutoFeed(int cube){
        ...
        if(cube == 1){
            autoFeedDistance -= 3;//feed a little bit closer for the middle cube
        }


        if(cube == 2){
            autoFeedDistance -= 6;//feed even closer for the right
            ...
            double targetAngleToTarget = Math.toRadians(30);
            //calculate the delta angleToGo
            ...
            if(deltaRadius > -25 && myAutoFeeder.isOkToOrbit()
                ...
                orbitAmount = -1.0 * Range.clip(angleToGo
...
//            movement_y *= scaleMovement;
    ...
    public double orbitModeFeeder(double xPower){
        ...
        if(Math.abs(SpeedOmeter.getDegPerSecond()) < 30){
            //increment the error sum
            autoFeedAngleErrorSum += (relativePointAngle/Math.toRadians(1.2))
        ...
        if(Math.abs(relativePointAngle) < Math.toRadians(30)){
            ...
            movement_y -= Range.clip(Math.abs(movement_x),0,1)*0.1;
        }
        ...
        double decelerationRadians = Math.toRadians(24);
        //max turn speed
...
//        minTurnPower *= 1.0 + Math.abs(movement_x) * 2.0;
        ...
        relativePointAngle -= (movement_x / 0.3) * Math.toRadians(7);
        ...
        double velocityAdjustedRelativePointAngle =
                AngleWrap(relativePointAngle-SpeedOmeter.currSlipAngle() * 1);
        ...
        movement_turn += autoFeedAngleErrorSum * 0.2;
