...
Java_org_firstinspires_ftc_robotcontroller_Vision_Vision_readBallPattern(JNIEnv *env, jclass type,
                                                                    jlong addrRgba,jdouble sample1X,jdouble sample2X, jdouble sample3X, jdouble sampleY) {
...
//    warpAffine(img, img, rot_mat, img.size());
    ...
    int sampleY_start = (int) ((sampleY-sampleRadius) / scaleSize);
    int sampleY_end = (int) ((sampleY +sampleRadius) / scaleSize);
    ...
    if(error1 <= 200 && error1 <= error2){
        circle(img,Point(sample1X,sampleY),30,Scalar(255,255,0),-1);
        whereIsCube = 0;
    }else{
        if(error2 <= 200 && error2 <= error1 ){
            circle(img,Point(sample2X,sampleY),30,Scalar(255,255,0),-1);
            whereIsCube = 1;
        }else{
            circle(img,Point(sample3X,sampleY),30,Scalar(255,255,0),-1);
            whereIsCube = 2;
        }
    }
